...
class ShowCatalog{
    ...
    public:
        ...
        void clear(ShowItem* Node);

        ShowItem* insert(ShowItem* node, string title, int year, string showRating, float userRating);

        void printPreOrder(ShowItem* node);

        ShowItem* find(ShowItem* node, const string& title);

        void searchByChar(ShowItem* node, char titleChar, bool& found);

        void countShowsWithRating(ShowItem* node, int& count, const string& showRating);

        void printLeaves(ShowItem* node);

        bool existsShowWithChar(ShowItem* node, char titleChar);
};
